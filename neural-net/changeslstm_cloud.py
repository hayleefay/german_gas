...
reframed.sort_values(by=[14, 13], inplace=True)

# and here is where we remove the last 30 days
day_30 = reframed[14].unique()[-30]
train = reframed.loc[reframed[14] < day_30]
test = reframed.loc[reframed[14] >= day_30]
...
inv_yy = inv_y[:, 0]
# calculate RMSE
rmse = sqrt(mean_squared_error(inv_yy, inv_yyhat))
print('Test RMSE: %.4f' % rmse)
print('\n\n')

inv_y['y'] = inv_yy
inv_y['y_hat'] = inv_yyhat

inv_y.to_csv("lstm_predictions.csv")
